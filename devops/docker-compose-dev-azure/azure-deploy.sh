...
resourcename="restaurant"  # lower case, only
resourcegroup="restaurant_rg"
...
# git clone https://github.com/apilogicserver/restaurant.git
# cd restaurant
...
az group create --name restaurant_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group restaurant_rg --sku S1 --is-linux
az container create --resource-group restaurant_rg --name restaurant --image apilogicserver/restaurant:latest --dns-name-label restaurant --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//restaurant.westus.azurecontainer.io:5656'
